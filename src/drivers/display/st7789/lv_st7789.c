...
{
    lv_display_t * disp = lv_st7789_create_display(hor_res, ver_res, send_cmd_cb, send_color_cb);
    if (disp == NULL)
    {
        return NULL;
    }

    lv_st7789_init(disp, flags);
...
{
    return lv_lcd_generic_mipi_create_display(hor_res, ver_res, send_cmd_cb, send_color_cb);
}
